public class Plateau {
    ...
    private void calculeAdjacence(){
        for (int i = 1; i <= this.hauteur; i++){
            for (int j = 1; j <= this.largeur; j++) computeAdja(i,j);
        }
    }
    ...
    public void afficheTout(){
        ...
        System.out.println("    * " + this.nbMines + " ".repeat(6 - ((int) Math.log10(this.nbMines))) + "/ " +
         this.nbDrapeaux + (this.nbDrapeaux == 0? "      " : (" ".repeat(6 -((int) Math.log10(this.nbDrapeaux))))) + "   *");
        System.out.println("    **********************");
        System.out.println();

        System.out.print("    ");
        for (int i = 1; i <= this.largeur; i++) System.out.print(i + " ".repeat((i <  10 ? 2 + (this.largeur/10 == 0 ? 0: 1): 2)));
        System.out.println();
        System.out.println("-".repeat(3*this.largeur + 4) + "-".repeat((this.largeur/10 == 0? 0 : 1)*this.largeur));

        for (int i = 0 ; i < this.hauteur; i++){
            System.out.print((char)((int) 'A' + i) + " | ");
            for (int j = 0 ; j < this.largeur ; j++ ){
                System.out.print((this.mines[i+1][j+1]? "*" : this.adja[i+1][j+1]) +  " ".repeat(2 + (this.largeur/10 == 0 ? 0: 1)));
            }
            System.out.println();
        }
    }
    ...
    public void revelerCase(int i, int j){
        ...
        if (this.adja[i][j] == 0){
            ...
            if (i-1 > 0 && j-1 > 0 && this.etats[i-1][j-1] == 0) this.revelerCase(i-1, j-1);
            if (i-1 > 0 && this.etats[i-1][j] == 0) this.revelerCase(i-1, j);
            if (i-1 > 0 && j+1 <= this.hauteur && this.etats[i-1][j+1] == 0) this.revelerCase(i-1, j+1);

            if (j-1 > 0 && this.etats[i][j-1] == 0) this.revelerCase(i, j-1);            
            if (j+1 <= this.hauteur && this.etats[i][j+1] == 0) this.revelerCase(i, j+1);

            if (i+1 <= this.largeur && j-1 > 0 && this.etats[i+1][j-1] == 0) this.revelerCase(i+1, j-1);
            if (i+1 <= this.largeur && this.etats[i+1][j] == 0) this.revelerCase(i+1, j);
            if (i+1 <= this.largeur && j+1 <= this.hauteur && this.etats[i+1][j+1] == 0) this.revelerCase(i+1, j+1);                
        }
    }
    ...
    public void afficheCourant(){
        ...
        for (int i = 0 ; i < this.hauteur; i++){
            System.out.print((char)((int) 'A' + i) + " | ");
            for (int j = 0 ; j < this.largeur ; j++ ){
                System.out.print((this.etats[i+1][j+1] == 0 ? "." : (this.etats[i+1][j+1] == 1? "!" : this.adja[i+1][j+1] )) 
    ...
    public boolean jeuPerdu(){
        for (int i = 0; i < this.hauteur; i++){
            for(int j = 0; j < this.largeur; j++){
                if (this.mines[i][j] && this.etats[i][j] == 2) return true;
            }
        }
        return false;
    }
    ...
    public boolean jeuGagne(){
        for (int i = 0; i < this.hauteur; i++){
            for(int j = 0; j < this.largeur; j++){
                if (this.mines[i][j] && this.etats[i][j] != 1) return false;
            }
        }
        return true;
    }
